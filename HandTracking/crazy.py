...
def cnn_model_fn(features, labels, mode):
    ...
    #Pooling Layer #1
    pool1 = tf.layers.max_pooling2d(inputs=dropout(conv1,
            training=mode == tf.estimator.ModeKeys.TRAIN), pool_size=[2, 2],
            strides=2)
    ...
    pool2 = tf.layers.max_pooling2d(inputs=dropout(conv2,
            training=mode == tf.estimator.ModeKeys.TRAIN), pool_size=[2, 2],
            strides=2)
    ...
    pool3 = tf.layers.max_pooling2d(inputs=dropout(conv3,
            training=mode == tf.estimator.ModeKeys.TRAIN), pool_size=[5, 5],
            strides=5)
    ...
    # Dense Layer
    pool2_flat = tf.reshape(norm3, [-1, 16 * 9 * 256])
