...
def mirror_label(box, axis=1, shape=(IMAGE_HEIGHT, IMAGE_WIDTH)):
    ...
    h, w, x, y = tf.split(tf.cast(box, tf.int32), [2,2,2,2], 1)
    ...
    return tf.concat((h, w, x, y), axis=1)
...
def randomlyFlipDataset(image, label):
    transforms = [flip_vertical, flip_horizontal]

    for trans in transforms:
        image, label = maybe_transform(trans,
                                        image,
                                        label)
