...
def cnn_model_fn(features, labels, mode):
    ...
    output = dense2(dropout)

    pred_box1, pred_box2 = tf.split(tf.reshape(output, [-1,4,2]), [1,1], axis=2)
    ...
    true_box1, true_box2 = tf.split(tf.reshape(labels, [4,2]), [1,1], axis=1)
...
def _decode_labels(label):
    ...
    label_tensor = tf.reshape(label_tensor, [8])
